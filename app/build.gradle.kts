...
// The direct/GitHub build can point at the maintainer's hosted checkout without
// putting a payment provider secret in the APK. Leave it blank until the HTTPS
// checkout and its webhook verifier are deployed.
val plusCheckoutUrl = providers.gradleProperty("plusCheckoutUrl").getOrElse("")
...
android {
    ...
    packaging {
        // LiteRT discovers the Google Tensor dispatch library through nativeLibraryDir.
    ...
    defaultConfig {
        ...
        // Client ID de la app de Spotify. Se toma de local.properties para no
...
// Builds the release APKs and copies them to `release-upload/` named the way the in-app updater
// expects (`PixlAudio-v<name>-vc<code>-<abi>.apk`, see data/update/GitHubReleaseParser). Upload
// those files unchanged to a GitHub release. Remember to raise APP_VERSION_CODE first.
tasks.register<Copy>("prepareGitHubRelease") {
    dependsOn("assembleRelease")
    val versionCode = (project.findProperty("APP_VERSION_CODE") as? String) ?: "1"
    val versionName = (project.findProperty("APP_VERSION_NAME") as? String) ?: "1.0.0"
    from(layout.buildDirectory.dir("outputs/apk/release"))
    include("*-release.apk")
    rename { original ->
        val abi = original.removePrefix("app-").removeSuffix("release.apk").removeSuffix("-")
        "PixlAudio-v$versionName-vc$versionCode-${abi.ifEmpty { "universal" }}.apk"
    }
    into(rootProject.layout.projectDirectory.dir("release-upload"))
}
...
dependencies {
    ...
    // Custom Tabs: Spotify's OAuth login refuses to run inside an embedded WebView.
    ...
    // Extracts playable YouTube stream URLs, including signature deciphering — the
    // NewPipe app's own library, updated continuously as YouTube changes its obfuscation.
    // Doing this by hand (see data/youtube/SignatureCipherSolver) stopped being viable once
    // YouTube restructured how the cipher is built; this is the maintained alternative.
    implementation(libs.newpipe.extractor)
    implementation(libs.kyant.backdrop)
    implementation(libs.kyant.shapes)
    // NewPipeExtractor la usa como implementation, así que sus clases no llegan a nuestro
    // classpath de compilación solo por depender de la librería; hay que declararla aparte.
    implementation(libs.nanojson)
    // WebSettingsCompat/WebViewFeature para el WebView que resuelve el poToken.
    implementation(libs.androidx.webkit)

    // LiteRT 2.2 supplies both the CPU Interpreter and the CompiledModel NPU API.
    // A Google Tensor AOT asset + bundled dispatch library are required for TPU execution.
    // Do not also include tensorflow-lite: its Interpreter classes duplicate this artifact.
    implementation(libs.litert)

    // Lyric alignment uses ONNX Runtime. An accepted NNAPI provider alone does not
