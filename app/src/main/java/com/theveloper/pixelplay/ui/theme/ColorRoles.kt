...
fun extractSeedColor(
    ...
    // `getPixels` throws on a HARDWARE-config bitmap (its pixels live in GPU memory and are
    // not CPU-readable). Callers are expected to request software bitmaps for extraction, but
    // the `runCatching` below would turn a mistake here into a silent wrong-colour fallback
    // rather than a visible failure — so normalise explicitly instead of relying on that.
    ...
    val workingBitmap = resizeForExtraction(readableBitmap, config.downscaleMaxDimension)
    ...
    // Recycle every intermediate we allocated, but never the caller's own bitmap. When the
    // HARDWARE normalisation above made a copy *and* resizeForExtraction downscaled it, that
    // is two intermediates, not one.
    if (workingBitmap !== bitmap) {
        workingBitmap.recycle()
    }
    if (readableBitmap !== bitmap && readableBitmap !== workingBitmap) {
