...
data class SettingsUiState(
    ...
    val audioQuality: AudioQuality = AudioQuality.ULTRASOUND,
    val playerAmbientStyle: PlayerAmbientStyle = PlayerAmbientStyle.BLENDED_COVER,
    ...
    val animatedLyricsBlurStrength: Float = 1.2f,
    val liquidGlassIntensity: Float = 0.55f,
    val appUiStyle: String = "LiquidGlass",
...
private sealed interface SettingsUiUpdate {
    ...
    data class Group2(
        ...
        val liquidGlassIntensity: Float,
        val appUiStyle: String,
        val disableBlurAllOver: Boolean,
        val showScrollbar: Boolean,
        val audioQuality: AudioQuality,
        val playerAmbientStyle: PlayerAmbientStyle
    ) : SettingsUiUpdate
}
...
class SettingsViewModel @Inject constructor(
     ...
     * indexado por posición (`values[17] as Float`), así que meterle un flujo más desplazaría los
     * índices de todo lo que viene detrás.
     */
    private val _storageUsage = MutableStateFlow<com.theveloper.pixelplay.data.cache.AudioCacheManager.StorageUsage?>(null)
    val storageUsage: StateFlow<com.theveloper.pixelplay.data.cache.AudioCacheManager.StorageUsage?> =
        _storageUsage.asStateFlow()

    fun refreshStorageUsage() {
        viewModelScope.launch { _storageUsage.value = audioCacheManager.storageUsage() }
    }

    /** Libera solo la caché automática: lo que el usuario descargó a mano no se toca. */
    fun clearAutoCache(onFreed: (Long) -> Unit) {
        viewModelScope.launch {
            val freed = audioCacheManager.clearAutoCache()
            _storageUsage.value = audioCacheManager.storageUsage()
            onFreed(freed)
        }
    }
    ...
    /**
     * The provider-agnostic counterpart to [currentAiBaseUrl] — that flow already reads
     * whichever provider is currently selected, but until this existed the only *write* path
     * was [onCustomBaseUrlChange], hardcoded to [AiProvider.CUSTOM]. That mismatch meant typing
     * a base URL while Ollama (or any future configurable-URL provider) was selected silently
     * saved it under Custom's key instead — the field looked like it worked, but the URL never
     * reached the provider it was entered for.
     */
    fun onAiBaseUrlChange(provider: AiProvider, baseUrl: String) {
        viewModelScope.launch {
    ...
    init {
        ...
        viewModelScope.launch {
            combine<Any?, SettingsUiUpdate.Group2>(
                ...
                userPreferencesRepository.liquidGlassIntensityFlow,
                userPreferencesRepository.appUiStyleFlow,
                userPreferencesRepository.audioQualityFlow,
                userPreferencesRepository.playerAmbientStyleFlow
            ) { values ->
                SettingsUiUpdate.Group2(
                    ...
                    liquidGlassIntensity = values[20] as Float,
                    appUiStyle = values[21] as String,
                    audioQuality = values[22] as AudioQuality,
                    playerAmbientStyle = values[23] as PlayerAmbientStyle
                )
            }.collect { update ->
                _uiState.update { state ->
                    state.copy(
                        ...
                        liquidGlassIntensity = update.liquidGlassIntensity,
                        appUiStyle = update.appUiStyle,
                        audioQuality = update.audioQuality,
                        playerAmbientStyle = update.playerAmbientStyle
                    )
                }
            }
        }
    ...
    fun setPlayerAmbientStyle(style: PlayerAmbientStyle) {
        viewModelScope.launch {
            userPreferencesRepository.setPlayerAmbientStyle(style)
        }
    }
    ...
    fun setAppUiStyle(style: String) {
        viewModelScope.launch {
            userPreferencesRepository.setAppUiStyle(style)
        }
    }

    fun setLiquidGlassIntensity(intensity: Float) {
        viewModelScope.launch {
            userPreferencesRepository.setLiquidGlassIntensity(intensity)
        }
    }
