...
class LyricsStateHolder @Inject constructor(
    ...
    private val automaticStudioManager: com.theveloper.pixelplay.data.worker.AutomaticStudioManager
) {
    ...
    fun initialize(
        ...
        automaticUpdatesJob?.cancel()
        automaticUpdatesJob = coroutineScope.launch {
            automaticStudioManager.lyricsUpdated.collect { updatedSongId ->
                val current = stablePlayerState.value.currentSong
                if (current != null &&
                    TaisInstrumentalIndex.canonicalSongId(current.id) ==
                    TaisInstrumentalIndex.canonicalSongId(updatedSongId)
                ) {
    ...
    fun fetchLyricsForSong(
        ...
        loadingJob = scope?.launch {
            ...
            if (!forcePickResults) {
                // The repository owns its IO dispatcher. Keeping this suspend call in the
                // owning scope also allows immediate stored results without an extra hop.
                val storedLyrics = try {
