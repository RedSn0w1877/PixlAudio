        ...
        Box(
            ...
            contentAlignment = Alignment.TopCenter
                ...
                Column(
                    ...
                    // Swipeable Content
                    //
                    // No animateContentSize here on purpose. It measures its child once,
                    // then smooths every SUBSEQUENT size change from that first reading —
                    // but HorizontalPager's first layout pass can under-report page 0's
                    // true height before its scroll state has settled, and the follow-up
                    // corrected size then reads as just another "change" to animate rather
                    // than the real initial size. The result: the box freezes at whatever
                    // that first undersized measurement was (here, roughly Row1Actions'
                    // height) and everything after it — Row2/Row3/ringtone — never
                    // becomes reachable, not even by scrolling, since the box itself
                    // never grows. Losing the page-height transition animation is a much
                    // smaller cost than a sheet that's stuck half-rendered.
                    Box(
                        modifier = Modifier
                            .fillMaxWidth()
                            ...
                            verticalAlignment = Alignment.Top
                                ...
                                when (page) {
                                    0 -> { // Options / Actions
                                        ...
                                        // Only BS-RoFormer is hidden here (showRoformerTools defaults to
                                        // false) — that specific render pipeline is still flaky for Beta 1.
                                        // Regular Instrumentalize and Sync Lyrics both stay on as normal.
                                        com.theveloper.pixelplay.presentation.components.tais.TaisStudioProgressCard(
                                            song = song,
                                            onInstrumentalReady = {
                                                Toast.makeText(
                                                    context,
                                                    "Instrumental ready for ${song.title} — play it from the lyrics screen.",
                                                    Toast.LENGTH_LONG
                                                ).show()
                                            },
                                            onLyricsReady = {
                                                Toast.makeText(
                                                    context,
                                                    "Lyrics synced for ${song.title}.",
                                                    Toast.LENGTH_LONG
                                                ).show()
                                            }
                                        )

                                        // Se descarta sola en canciones locales (ya suenan sin
                                        // conexión), así que no hace falta condicionarla aquí.
                                        OfflineDownloadCard(
                                            song = song,
                                            onDownload = { songInfoViewModel.downloadSong(it) },
                                            onRemoveDownload = { songInfoViewModel.removeDownloadedSong(it) }
                                        )
    ...
    Row(
        ...
        Box(
            ...
            Icon(
                ...
                tint = favoriteButtonContentColor
            )
        }
...
private fun Row4Actions(
    ...
    Row(
        ...
        val watchEnabled = shouldOfferWatchTransfer && !isSendingToWatch
        val watchContainerColor = if (isPixelPlayWatchAvailable) {
            sendToWatchContainerColor
        } else {
            MaterialTheme.colorScheme.surfaceContainerHigh
        }
        val watchContentColor = if (isPixelPlayWatchAvailable) {
        ...
        Row(
            ...
            CompositionLocalProvider(LocalContentColor provides watchContentColor) {
