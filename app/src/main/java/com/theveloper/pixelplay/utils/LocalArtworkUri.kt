...
object LocalArtworkUri {
    ...
    private val SHARED_SONG_URI = Regex("^content://[^/]+\\.artwork/song/(-?\\d+)(?:[?#].*)?$", RegexOption.IGNORE_CASE)
    ...
    fun parseSongIdFromVolatileArtworkUri(uriString: String?): Long? {
        ...
        SHARED_SONG_URI.matchEntire(uriString)?.groupValues?.get(1)?.toLongOrNull()?.let { return it }
