...
object LyricsUtils {
    ...
    fun parseLyrics(lyricsText: String?): Lyrics {
        ...
        if (normalizedInput.startsWith("{")) {
            com.theveloper.pixelplay.data.model.LyricsDocCodec.decode(normalizedInput)?.let { return it.toLyrics() }
            // JSON objects can also be YRC metadata lines; allow the YRC detector below.
            if (normalizedInput.contains("\"pixelplay-lyrics\"")) return Lyrics(plain = emptyList(), synced = emptyList())
        }
        if (normalizedInput.startsWith("[") && normalizedInput.contains("\"ts\"")) {
            return com.theveloper.pixelplay.data.network.lyrics.WordSyncTranspilers.richSync(normalizedInput)?.toLyrics()
                ?: Lyrics(plain = emptyList(), synced = emptyList())
        }
        if (Regex("(?m)^\\[\\d+,\\d+]\\(").containsMatchIn(normalizedInput)) {
            return com.theveloper.pixelplay.data.network.lyrics.WordSyncTranspilers.yrc(normalizedInput)?.toLyrics()
                ?: Lyrics(plain = emptyList(), synced = emptyList())
        }
    ...
    internal fun pairTranslationLines(lines: List<SyncedLine>): List<SyncedLine> {
        ...
        // A failed alignment can stamp an entire song at zero. Those are separate
        // original lines, not translations; pairing them loses half the text on resync.
        if (lines.all { it.time == 0 } && lines.any { !it.words.isNullOrEmpty() } &&
            lines.flatMap { it.words.orEmpty() }.all { it.time == 0 }) return lines
        val result = mutableListOf<SyncedLine>()
        var i = 0
        while (i < lines.size) {
            val current = lines[i]
            val next = lines.getOrNull(i + 1)
            if (next != null && next.time == current.time && next.words.isNullOrEmpty() && current.translation == null && current.line.isNotBlank() && next.line.isNotBlank()) {
