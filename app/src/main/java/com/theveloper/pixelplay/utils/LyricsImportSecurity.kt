...
object LyricsImportSecurity {
    ...
    private enum class LyricsDocumentFormat(
        ...
        JSON("json", setOf("application/json", "text/plain", "application/octet-stream")),
        YRC("yrc", setOf("text/plain", "application/octet-stream")),
        TTML(
            extension = "ttml",
            allowedMimeTypes = setOf(
    ...
    fun validateImportedLrcContent(rawText: String): LyricsImportValidationResult {
        ...
        val limit = if (sanitized.trimStart().startsWith("{")) MAX_TTML_FILE_BYTES else MAX_LYRICS_TEXT_CHARS
    ...
    fun messageFor(reason: LyricsImportFailureReason): String {
        return when (reason) {
            LyricsImportFailureReason.UNSUPPORTED_EXTENSION ->
                "Supported lyrics files: .lrc, .ttml, .yrc and PixelPlay .json."
    ...
    private fun normalizationCandidates(
        ...
        val candidates = when (format) {
            LyricsDocumentFormat.JSON, LyricsDocumentFormat.YRC -> listOf(sanitizeImportedLyrics(decoded))
    ...
    private fun resolveDocumentFormat(fileName: String?): LyricsDocumentFormat? {
        val lowerName = fileName?.lowercase() ?: return null
        val normalized = lowerName.substringAfterLast('.', missingDelimiterValue = "")
    ...
    private fun maxFileBytesFor(format: LyricsDocumentFormat): Int =
        if (format == LyricsDocumentFormat.TTML || format == LyricsDocumentFormat.JSON) MAX_TTML_FILE_BYTES else MAX_LYRICS_FILE_BYTES
