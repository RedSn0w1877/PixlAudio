...
class AudioCacheManager @Inject constructor(
    ...
    // WorkManager poda su historial, así que el motivo del fallo no puede vivir solo en el
    // outputData del worker: se perdería justo cuando el usuario vuelve a mirar la pantalla.
    private val failurePreferences = context.getSharedPreferences("offline_download_failures", Context.MODE_PRIVATE)
    private val downloadFailures = MutableStateFlow(
        failurePreferences.all.mapNotNull { (songId, reason) ->
            (reason as? String)?.let { songId to it }
        }.toMap()
    )
    ...
    init {
        managerScope.launch {
            ...
            songCacheDao.getAllCacheStatesFlow()
                .combine(downloadFailures) { rows, failures -> rows to failures }
                .collect { (rows, failures) ->
                    val states = mutableMapOf<String, Int>()
                    rows.forEach { row ->
                        val state = when {
                            row.isComplete && row.isPermanent -> SongCacheState.DOWNLOADED
                            row.isComplete -> SongCacheState.CACHED
                            // Incompleta y automática = todavía no ha sonado entera; no es algo
                            // que el usuario haya pedido, así que no se muestra como nada.
                            !row.isPermanent -> null
                            failures.containsKey(row.songId) -> SongCacheState.FAILED
                            else -> SongCacheState.DOWNLOADING
                        }
                        if (state != null) states[row.songId] = state
                    }
                    // Un fallo cuya fila ya no existe (borrada entre medias) sigue mereciendo
                    // marcarse: si no, el botón de reintentar desaparecería sin explicación.
                    failures.keys.forEach { songId ->
                        if (!states.containsKey(songId)) states[songId] = SongCacheState.FAILED
                    }

                    SongCacheStateCache.update(states)
                    SongCacheStateCache.updateFailureReasons(failures)
                }
        }
    }

    /**
     * Marca una descarga como fallida de forma duradera. La llama [SongDownloadWorker] cuando se
     * rinde: ese worker devuelve `Result.success()` a propósito (un fallo real cancelaría el resto
     * de la cola compartida), así que el estado del trabajo no sirve para detectar el fallo.
     */
    fun markDownloadFailed(songId: String, reason: String) {
        failurePreferences.edit().putString(songId, reason).apply()
        downloadFailures.value = downloadFailures.value + (songId to reason)
    }

    /** Limpia el fallo al reintentar o al completarse la descarga. */
    fun clearDownloadFailure(songId: String) {
        if (!downloadFailures.value.containsKey(songId)) return
        failurePreferences.edit().remove(songId).apply()
        downloadFailures.value = downloadFailures.value - songId
    }
    ...
    fun requestDownload(songId: String) {
        // Reintentar es volver a pedirla: el fallo anterior deja de valer en cuanto se reintenta.
        clearDownloadFailure(songId)
        managerScope.launch {
    ...
    suspend fun downloadAndCache(
        ...
        downloadLocks.getOrPut(songId) { Mutex() }.withLock download@ {
            ...
            try {
                // Solo se publica el progreso de una descarga explícita: la caché automática pasa
                // sola por aquí al escuchar una canción y su porcentaje no es algo que el usuario
                // haya pedido ver.
                val contentType = AudioDownloadTransfer.fetch(client, proxyUrl, pending) { percent ->
                    if (isPermanent) DownloadProgressCache.update(songId, percent)
                }
                ...
                pending.delete()
                DownloadProgressCache.clear(songId)
            }
        }
    }
    ...
    /** Lo que ocupa el audio local, separado por quién lo puso ahí. */
    data class StorageUsage(
        val downloadedBytes: Long,
        val downloadedCount: Int,
        val autoCachedBytes: Long,
        val autoCacheLimitBytes: Long = MAX_AUTO_CACHE_BYTES
    )

    suspend fun storageUsage(): StorageUsage = mutationLock.withLock {
        StorageUsage(
            downloadedBytes = songCacheDao.getDownloadsTotalBytes(),
            downloadedCount = songCacheDao.getDownloadsCount(),
            autoCachedBytes = songCacheDao.getAutoCacheTotalBytes()
        )
    }

    /**
     * Borra solo la caché automática. Las descargas explícitas no se tocan: el usuario las pidió,
     * así que liberar espacio nunca debe llevarse por delante lo que eligió guardar. Devuelve los
     * bytes liberados.
     */
    suspend fun clearAutoCache(): Long = mutationLock.withLock {
        var freed = 0L
        for (entry in songCacheDao.getAutoCacheEntriesOldestFirst()) {
            ...
            if (file.exists() && !file.delete()) {
                Timber.w("Could not delete cached audio for %s", current.songId)
                continue
            }
            songCacheDao.delete(current.songId)
            freed += current.sizeBytes
        }
        freed
    }
