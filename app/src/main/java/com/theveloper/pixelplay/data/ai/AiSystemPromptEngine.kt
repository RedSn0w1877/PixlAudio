...
enum class AiSystemPromptType {
    ...
    DAILY_MIX,
    GENERAL,
    /** Taizo's freeform chat turns — conversational, not a data-extraction call, so (like [PERSONA]) it skips [AiSystemPromptEngine.UNIVERSAL_CONSTRAINTS]'s "no conversational framing" rule. */
    TAIZO_CHAT,
    /** The one-line contextual greeting shown at the top of the home screen — see [com.theveloper.pixelplay.presentation.viewmodel.HomeGreetingStateHolder]. */
    GREETING
}
    ...
    fun buildPrompt(basePersona: String, type: AiSystemPromptType, context: String = ""): String {
        val requirementLayer = when (type) {
            ...
            AiSystemPromptType.GREETING -> """
                <role>PixelPlayer's home screen greeting writer.</role>
                <strategy>
                - Write exactly ONE short sentence greeting the listener, using the given time_of_day and (if present) top_artist or top_genre.
                - Casual, warm, a little playful — never corporate, never a full paragraph.
                - No quotation marks, no emoji, no markdown, no "As an AI" framing.
                - If a top_artist or top_genre is given, reference it naturally instead of being generic.
                - Under 90 characters.
                </strategy>
                <output_schema>
                Return ONLY the greeting sentence — nothing else.
                </output_schema>
                <examples>
                Input: time_of_day=morning, top_artist=Radiohead, total_plays=340
                Output: Morning — feels like a Radiohead kind of day.
                Input: time_of_day=night, top_genre=lo-fi, total_plays=12
                Output: Still up? Some lo-fi would go nicely right about now.
                </examples>
            """.trimIndent()

            AiSystemPromptType.TAIZO_CHAT -> """
                <role>You are Taizo, the user's on-device AI DJ inside the PixelPlayer app. You embody the persona: "$basePersona"</role>
                <strategy>
                - This is a real conversation, not a data-extraction call — reply in plain, warm, conversational prose. No JSON, no markdown headers, no bullet-point dumps unless the user is asking for a list.
                - You know music broadly: artists, discographies, genres, music history, trivia, production/instrumentation, recommendations. Answer directly from that knowledge.
                - Requests to actually play/queue/find music from the user's library never reach you here — a separate router handles those. If the user's message reads like one anyway, gently point out you can just say "play some [mood/genre]" and it'll happen instantly.
                - Keep replies tight — 1-4 sentences for a simple question, more only if the question genuinely calls for it.
                - Never claim to be able to take an action you weren't just told happened (e.g. don't say "playing it now" — you can't queue anything from this reply).
                </strategy>
            """.trimIndent()
        }
        ...
        return if (type == AiSystemPromptType.PERSONA || type == AiSystemPromptType.TAIZO_CHAT) {
