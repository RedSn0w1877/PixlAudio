...
object SourceType {
    ...
    /** Derive source type from a content URI string (fallback for conversion). */
    fun fromContentUri(uri: String): Int = when {
        uri.startsWith("spotify://") -> SPOTIFY
        else -> LOCAL
    }
}
...
private fun SongEntity.toSongInternal(artists: List<ArtistRef>): Song {
    return Song(
        ...
        year = this.year,
        // Parse the Spotify track id back out of the content URI
        spotifyId = if (this.contentUriString.startsWith("spotify://")) {
            this.contentUriString.removePrefix("spotify://")
        } else null,
