...
interface MusicDao {
    ...
    suspend fun getAllSpotifySongIds(): List<Long>
    ...
    @Transaction
    suspend fun clearAllSpotifySongs() {
        val spotifySongIds = getAllSpotifySongIds()
        if (spotifySongIds.isEmpty()) return
        deleteSongsAndRelatedData(spotifySongIds)
    }
    ...
    @Query("""
        SELECT * FROM songs
        WHERE (:applyDirectoryFilter = 0 OR id < 0 OR parent_directory_path IN (:allowedParentDirs))
        AND (title LIKE '%' || :query || '%' OR artist_name LIKE '%' || :query || '%' OR genre LIKE '%' || :query || '%')
        ORDER BY title ASC
    """)
    fun searchSongsLike(
        query: String,
    ...
    @Query("""
        ...
        WHERE (albums.title LIKE '%' || :query || '%' OR albums.artist_name LIKE '%' || :query || '%')
    ...
    @Query("""
        ...
        INNER JOIN songs ON songs.id = song_artist_cross_ref.song_id
    ...
    suspend fun rebuildMusicDataWithCrossRefs(
        ...
        // A MediaStore rebuild owns local rows only. Streaming mirrors may be the
        // only library entry for a retained download while the device is offline.
        getAllMediaStoreSongIds().chunked(CROSS_REF_BATCH_SIZE).forEach { chunk ->
            deleteCrossRefsBySongIds(chunk)
            deleteSongsByIds(chunk)
        }
