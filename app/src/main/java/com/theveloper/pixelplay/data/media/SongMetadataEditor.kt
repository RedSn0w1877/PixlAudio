...
class SongMetadataEditor(
    ...
    suspend fun editSongMetadata(
        ...
        try {
            ...
            val fileUpdateSuccess = if (!fileExists) {
                if (isCloudSong) {
                    Timber.tag(TAG)
                        .w("METADATA_EDIT: Cloud file not present locally (streaming). Skipping file tags, updating DB only.")
                    true
                } else {
                    ...
                    if (writeOk) {
                        ...
                        try {
                            ...
                            if (originalFile.canWrite()) {
                                ...
                                val uri = if (!isCloudSong) MediaStorePermissionHelper.getMediaStoreUri(context, songId) else null
                                if (uri != null) {
            ...
            if (isCloudSong) {
                // Las canciones en la nube solo viven en la tabla unificada; no hay tabla
                // por fuente que actualizar además de la escritura ya hecha arriba.
            } else {
     ...
     * o remota). Escribir tags ID3v2 to an MP4 container corrupts it irreversibly, so the
