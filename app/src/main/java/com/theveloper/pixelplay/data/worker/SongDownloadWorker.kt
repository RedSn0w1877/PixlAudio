...
class SongDownloadWorker @AssistedInject constructor(
    ...
    override suspend fun doWork(): Result = withContext(Dispatchers.IO) {
        ...
        if (file == null) {
            ...
            val reason = "Could not download this song after 4 attempts. Tap Download to try again."
            // El estado del trabajo aquí es SUCCESS a propósito (ver arriba), así que la pantalla
            // no puede enterarse por WorkManager: solo una descarga explícita se marca como
            // fallida, porque la caché automática falla sola y en silencio por diseño.
            if (isPermanent) audioCacheManager.markDownloadFailed(songId, reason)
            return@withContext Result.success(workDataOf(OUTPUT_FAILURE_REASON to reason))
        }
        audioCacheManager.clearDownloadFailure(songId)
