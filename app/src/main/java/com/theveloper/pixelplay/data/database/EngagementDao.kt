...
interface EngagementDao {
    ...
    /**
     * Las más escuchadas de entre un conjunto concreto de canciones.
     *
     * Acotado a los ids que interesan en vez de leer la tabla entera y filtrar en memoria:
     * la ficha de un artista sólo necesita saber de sus propias canciones.
     */
    @Query("""
        SELECT song_id, play_count FROM song_engagements
        WHERE song_id IN (:songIds) AND play_count > 0
        ORDER BY play_count DESC
        LIMIT :limit
    """)
    suspend fun getTopPlayedAmong(songIds: List<String>, limit: Int): List<SongPlayCount>
...
data class SongPlayCount(
    @androidx.room.ColumnInfo(name = "song_id") val songId: String,
