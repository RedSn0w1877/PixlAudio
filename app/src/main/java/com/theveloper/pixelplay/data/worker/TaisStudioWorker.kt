...
class TaisStudioWorker @AssistedInject constructor(
    ...
    override suspend fun doWork(): Result = if (automatic) {
        automaticEnvironment.runQuietly(AutomaticStudioKind.LYRICS) { performWork() }
    } else performWork()
    ...
    private suspend fun performWork(): Result = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            syncMutex.withLock {
                ...
                // here would repeat the same network lookup for line-only results.
                val state = if (automatic) {
                    val stored = musicRepository.getStoredLyrics(song)?.first
                        ?: song.lyrics?.let(com.theveloper.pixelplay.utils.LyricsUtils::parseLyrics)
                    lyricsAligner.alignmentStateFor(stored)
                } else lyricsAligner.loadAndClassify(
                ...
                if (online != null && !online.lyrics.synced.isNullOrEmpty()) {
                    ...
                    val existingWordLines = (state as? TaisLyricsAligner.AlignmentState.LineSyncedOnly)
                        ?.lines.orEmpty().count { line -> line.words.orEmpty().any { it.time > 0 } }
                    if (automatic && existingWordLines > 0 && wordLines == 0) {
                        reportProgress(STAGE_DONE, 100, "Kept the existing word timings for ${song.title}")
                ...
                val sourceAudioPath = if (automatic) {
                    AutomaticStudioEnvironment.localAudio(song, audioCacheManager)
                        ?: return@withLock Result.success(resultData(OUTCOME_SKIPPED, false,
                            "No word-synced catalog match; acoustic sync waits for audio already on this phone"))
                } else song.spotifyId?.let { spotifyId ->
    ...
    private suspend fun reportProgress(stage: String, percent: Int, detailText: String) {
        ...
        if (automatic) return // Unattended work is visible in-app only; never posts a notification.
        setForeground(
