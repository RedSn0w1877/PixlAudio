...
/**
 * User-selectable streaming audio quality tier. Caps the bitrate
 * [com.theveloper.pixelplay.data.youtube.YouTubeStreamResolver.pickBestAudio] will pick among the
 * candidate formats a video offers — it can only choose among what YouTube actually serves for
 * that video, it cannot demand a higher bitrate than exists. There is no lossless/hi-res stream
 * available by any route (see CLAUDE.md), so [ULTRASOUND] means "no cap, take the best available"
 * rather than an actual ultrasonic/hi-res guarantee.
 *
 * @property maxBitrateKbps upper bound on selectable bitrate, or null for no cap.
 */
enum class AudioQuality(val maxBitrateKbps: Int?, val label: String) {
    LOW(96, "Low"),
    STANDARD(160, "Standard"),
    HIGH(256, "High"),
    ULTRASOUND(null, "Ultrasound")
}

/**
 * The full player's ambient background style — these are exclusive, not layered: picking one
 * replaces whatever was showing before, the same way [ThemePreference] or carousel style work.
 */
enum class PlayerAmbientStyle(val label: String) {
    OFF("Off"),
    BLENDED_COVER("Blended cover"),
    FLOWING_GRADIENT("Flowing gradient"),
    LOW_POLY_MESH("Low-poly mesh"),
    AUDIO_WAVEFORM("Audio waveform")
}

/**
 * Which wire protocol the "Render Studio Master (BS-RoFormer)" button should speak — these are
 * not interchangeable (see [com.theveloper.pixelplay.data.tais.stems.BsRoformerApiClient] and
 * [com.theveloper.pixelplay.data.tais.stems.DirectPostStemApiClient] class docs), so the base
 * URL/route/API key fields get reinterpreted per backend rather than duplicated per type.
 */
enum class TaisRoformerBackendType(val label: String) {
    GRADIO_SPACE("Gradio Space"),
    DIRECT_POST("Direct POST / FastAPI")
}
...
class UserPreferencesRepository @Inject constructor(
    ...
    private object PreferencesKeys {
        val AUTOMATIC_LYRICS = booleanPreferencesKey("automatic_lyrics")
        val AUTOMATIC_INSTRUMENTALS = booleanPreferencesKey("automatic_instrumentals")
        ...
        val TAIS_VOCAL_ATTENUATION = floatPreferencesKey("tais_vocal_attenuation")
        val TAIS_ROFORMER_BASE_URL = stringPreferencesKey("tais_roformer_base_url")
        val TAIS_ROFORMER_API_NAME = stringPreferencesKey("tais_roformer_api_name")
        ...
        val PLAYER_AMBIENT_STYLE = stringPreferencesKey("player_ambient_style")

        // Home greeting (AI-generated, cached for the day it was produced)
        val HOME_GREETING_TEXT = stringPreferencesKey("home_greeting_text")
        val HOME_GREETING_DATE = stringPreferencesKey("home_greeting_date")
    ...
    val audioQualityFlow: Flow<AudioQuality> =
        pref { preferences ->
            preferences[PreferencesKeys.AUDIO_QUALITY]
                ?.let { runCatching { AudioQuality.valueOf(it) }.getOrNull() }
                ?: AudioQuality.ULTRASOUND
        }
    ...
    val playerAmbientStyleFlow: Flow<PlayerAmbientStyle> =
        pref { preferences ->
            preferences[PreferencesKeys.PLAYER_AMBIENT_STYLE]
                ?.let { runCatching { PlayerAmbientStyle.valueOf(it) }.getOrNull() }
                ?: PlayerAmbientStyle.BLENDED_COVER
        }

    suspend fun setPlayerAmbientStyle(style: PlayerAmbientStyle) {
        dataStore.edit { it[PreferencesKeys.PLAYER_AMBIENT_STYLE] = style.name }
    }

    val homeGreetingTextFlow: Flow<String> =
        pref { it[PreferencesKeys.HOME_GREETING_TEXT] ?: "" }

    val homeGreetingDateFlow: Flow<String> =
        pref { it[PreferencesKeys.HOME_GREETING_DATE] ?: "" }

    suspend fun setHomeGreeting(date: String, text: String) {
        dataStore.edit {
            it[PreferencesKeys.HOME_GREETING_DATE] = date
            it[PreferencesKeys.HOME_GREETING_TEXT] = text
        }
    }
    ...
    // TAIS Engine 2's DSP fallback slider — 0f (off) to 1f (max mid-channel/vocal attenuation).
    val taisVocalAttenuationFlow: Flow<Float> =
        pref { (it[PreferencesKeys.TAIS_VOCAL_ATTENUATION] ?: 0f).coerceIn(0f, 1f) }

    suspend fun setTaisVocalAttenuation(amount: Float) {
        dataStore.edit { it[PreferencesKeys.TAIS_VOCAL_ATTENUATION] = amount.coerceIn(0f, 1f) }
    }

    // BS-RoFormer / Mel-Band RoFormer render backend — a real Gradio Spaces REST API target (see
    // BsRoformerApiClient doc). Blank base URL means the "Render Studio Master (BS-RoFormer)"
    // button stays disabled rather than silently falling back to anything else.
    val taisRoformerBaseUrlFlow: Flow<String> =
        pref { it[PreferencesKeys.TAIS_ROFORMER_BASE_URL] ?: "" }

    suspend fun setTaisRoformerBaseUrl(url: String) {
        dataStore.edit { it[PreferencesKeys.TAIS_ROFORMER_BASE_URL] = url.trim() }
    }

    /** The Gradio function route to call, e.g. "/predict" — every Space names this differently, so it can't be assumed. */
    val taisRoformerApiNameFlow: Flow<String> =
        pref { it[PreferencesKeys.TAIS_ROFORMER_API_NAME] ?: "/predict" }

    suspend fun setTaisRoformerApiName(apiName: String) {
        dataStore.edit { it[PreferencesKeys.TAIS_ROFORMER_API_NAME] = apiName.trim() }
    }

    /** Optional bearer token for gated/private Hugging Face Spaces. */
    val taisRoformerApiKeyFlow: Flow<String> =
        pref { it[PreferencesKeys.TAIS_ROFORMER_API_KEY] ?: "" }

    suspend fun setTaisRoformerApiKey(apiKey: String) {
        dataStore.edit { it[PreferencesKeys.TAIS_ROFORMER_API_KEY] = apiKey.trim() }
    }

    /**
     * Some Spaces' separation function takes a second positional argument after the audio file
     * (e.g. a stem-variant dropdown) that errors out server-side if the call omits it entirely,
     * even when the Space's own docs list it as optional with a default — confirmed against
     * `hugging-apps-bs-roformer-leap-audio-separator`, which needs exactly "Standard — Vocals"
     * here. Blank means send only the audio file, matching a Space with no such extra argument.
     */
    val taisRoformerExtraArgFlow: Flow<String> =
        pref { it[PreferencesKeys.TAIS_ROFORMER_EXTRA_ARG] ?: "" }

    suspend fun setTaisRoformerExtraArg(value: String) {
        dataStore.edit { it[PreferencesKeys.TAIS_ROFORMER_EXTRA_ARG] = value }
    }

    /** Defaults to Gradio Space since that's what every public HF Space is. */
    val taisRoformerBackendTypeFlow: Flow<TaisRoformerBackendType> =
        pref { preferences ->
            preferences[PreferencesKeys.TAIS_ROFORMER_BACKEND_TYPE]
                ?.let { runCatching { TaisRoformerBackendType.valueOf(it) }.getOrNull() }
                ?: TaisRoformerBackendType.GRADIO_SPACE
        }

    suspend fun setTaisRoformerBackendType(type: TaisRoformerBackendType) {
        dataStore.edit { it[PreferencesKeys.TAIS_ROFORMER_BACKEND_TYPE] = type.name }
    }
...
suspend fun markDirectoryRulesVersionApplied(version: Int) {
    ...
    val automaticLyricsEnabledFlow: Flow<Boolean> = pref { it[PreferencesKeys.AUTOMATIC_LYRICS] ?: true }
    val automaticInstrumentalsEnabledFlow: Flow<Boolean> = pref { it[PreferencesKeys.AUTOMATIC_INSTRUMENTALS] ?: true }

    suspend fun setAutomaticLyricsEnabled(enabled: Boolean) {
        dataStore.edit { it[PreferencesKeys.AUTOMATIC_LYRICS] = enabled }
    }
    suspend fun setAutomaticInstrumentalsEnabled(enabled: Boolean) {
        dataStore.edit { it[PreferencesKeys.AUTOMATIC_INSTRUMENTALS] = enabled }
    }
    ...
    val animatedLyricsBlurStrengthFlow: Flow<Float> =
        // Keep in sync with LyricsSheet.kt's own read of the same key — both must agree on the
        // unset-value fallback or the Settings slider and the actual rendered blur disagree.
        pref { it[PreferencesKeys.ANIMATED_LYRICS_BLUR_STRENGTH] ?: 1.2f }
    ...
    suspend fun setLiquidGlassIntensity(intensity: Float) {
        dataStore.edit { it[PreferencesKeys.LIQUID_GLASS_INTENSITY] = intensity.coerceIn(0f, 1f) }
    }
    ...
    suspend fun setAppUiStyle(style: String) {
        dataStore.edit { it[PreferencesKeys.APP_UI_STYLE] = style }
    }
