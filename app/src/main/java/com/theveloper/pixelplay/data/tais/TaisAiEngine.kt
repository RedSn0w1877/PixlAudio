...
class TaisAiEngine @Inject constructor(
    ...
    private suspend fun prepareNpu() {
        try {
            ...
            if ("stem_separation_tensor_g5.json" !in manifests || "stem_separation_tensor_g5.tflite" !in manifests) {
                npuDetail = "Tensor G5 detected. The TPU runtime is included, but the compiler-produced separation model is not bundled. CPU is active."
                return
            }
            val json = context.assets.open(NPU_MANIFEST).bufferedReader().use { it.readText() }
            val sourceHash = assetHash(STEM_ASSET)
            val compiledHash = assetHash(NPU_ASSET)
            val manifest = TaisNpuModelContract.parseAndValidate(json, compiledHash, sourceHash)
            currentCoroutineContext().ensureActive()
            val compiledFile = materializeCompiledAsset(compiledHash)
            currentCoroutineContext().ensureActive()
