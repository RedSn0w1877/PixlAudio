...
class TaisStemSeparator @Inject constructor(
    ...
    private suspend fun separateChannelPair(
        ...
        val fft = Fft.Workspace(FFT_SIZE)
        ...
        while (chunkStart < totalFrames) {
            ...
            inputBuffer.clear()
            writeChunkStft(inputBuffer, leftPadded, window, chunkStart, chunkLen, re, im, fft, planeOffset = 0)
            writeChunkStft(inputBuffer, rightPadded, window, chunkStart, chunkLen, re, im, fft, planeOffset = 2)
            inputBuffer.rewind()
