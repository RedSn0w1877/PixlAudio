...
class PhoneDirectWatchTransferCoordinator @Inject constructor(
    ...
    private fun resolveStreamUrl(song: Song): String? {
        val rawUri = song.contentUriString
        val uri = runCatching { rawUri.toUri() }.getOrNull() ?: return null
        // Solo quedan URLs directas; las fuentes por proxy se resolvían aquí antes.
        return if (uri.scheme?.lowercase() in setOf("http", "https")) rawUri else null
    }
