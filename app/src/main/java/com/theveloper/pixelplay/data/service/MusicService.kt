...
import com.theveloper.pixelplay.data.stream.CloudStreamUris
...
class MusicService : MediaLibraryService() {
    ...
    @Inject
    lateinit var spotifyStreamProxy: com.theveloper.pixelplay.data.spotify.SpotifyStreamProxy
    @Inject
    ...
    // Mirrors a rendered instrumental in lockstep with the main player so the "Magic
    // Instrumentalize" toggle crossfades volume instead of hard-swapping the media item — see
    // the class doc for why it never touches audio focus or the MediaSession.
    private val instrumentalCrossfadeController by lazy {
        com.theveloper.pixelplay.data.service.player.InstrumentalCrossfadeController(
            context = this,
            scope = serviceScope
        )
    }
    ...
    override fun onCreate() {
        ...
        serviceScope.launch {
            userPreferencesRepository.taisVocalAttenuationFlow.collect { amount ->
                engine.setVocalAttenuation(amount)
            }
        }
        ...
        val callback = object : MediaLibrarySession.Callback {
            override fun onConnect(
                ...
                val customCommands = listOf(
                    ...
                    MusicNotificationProvider.CUSTOM_COMMAND_SET_STUDIO_AUDIO_OVERRIDE,
            ...
            override fun onCustomCommand(
                ...
                when (customCommand.customAction) {
                    ...
                    MusicNotificationProvider.CUSTOM_COMMAND_SET_STUDIO_AUDIO_OVERRIDE -> {
                        val path = args.getString(MusicNotificationProvider.EXTRA_STUDIO_AUDIO_PATH)
                        return serviceScope.future {
                            applyStudioAudioOverride(session, path)
    ...
    private val playerListener = object : Player.Listener {
        ...
        override fun onIsPlayingChanged(isPlaying: Boolean) {
            ...
            if (isPlaying) automaticStudioManager.onSongChanged(player.currentMediaItem?.mediaId)
            updateNextStreamPrefetch()
            syncLocalListeningStatsFromPlayer(player)
        ...
        override fun onPositionDiscontinuity(
            ...
            if (reason == Player.DISCONTINUITY_REASON_SEEK) {
                instrumentalCrossfadeController.onMainSeek(newPosition.positionMs)
            }
        ...
        override fun onMediaItemTransition(mediaItem: MediaItem?, reason: Int) {
            automaticStudioManager.onSongChanged(mediaItem?.mediaId)
            updateNextStreamPrefetch()
            instrumentalCrossfadeController.onSongChanged(
                mediaItem?.mediaId,
                mediaSession?.player ?: engine.masterPlayer
            )
        ...
        override fun onPlayerError(error: PlaybackException) {
            ...
            serviceScope.launch {
                ...
                // `error.message` es solo la categoría ("Source error"). La causa concreta
                // vive en la cadena de excepciones: un 403, un formato irreconocible y un
                // esquema sin resolver salen todos con el mismo texto y necesitan arreglos
                // distintos.
                PlaybackErrorReporter.report(
                    PlaybackErrorInfo(
                        trackTitle = trackTitle,
                        errorCodeName = error.errorCodeName,
                        causeChain = describeCauseChain(error),
                        failingUri = currentMediaItem?.localConfiguration?.uri?.toString()
                    )
                )
        ...
        /** Aplana la cadena de causas; la más profunda suele ser la real. */
        private fun describeCauseChain(error: Throwable): String {
            val parts = mutableListOf<String>()
            var current: Throwable? = error
            var depth = 0
            while (current != null && depth < 6) {
                val message = current.message?.take(180).orEmpty()
                parts += buildString {
    ...
    override fun onDestroy() {
        ...
        instrumentalCrossfadeController.teardown()
        ...
        for (index in 0 until mediaItemCount) {
            ...
            snapshotItems.add(
                PlaybackQueueItemSnapshot(
                    ...
                    // Se guarda la URI canónica, nunca la URL del proxy local: esa lleva
                    // dentro un puerto que solo existe en el arranque actual del servidor.
                    // Persistirla dejaba la cola envenenada para la sesión siguiente.
                    uri = CloudStreamUris.canonicalize(uri),
    ...
    private fun buildMediaItemFromSnapshot(snapshotItem: PlaybackQueueItemSnapshot): MediaItem? {
        ...
        // Repara colas guardadas por versiones anteriores, que sí escribieron la URL del
        // proxy con su puerto. Sin esto, quien ya tuviera una cola envenenada seguiría
        // chocando contra un puerto cerrado por mucho que se arregle el guardado.
        val item = snapshotItem.copy(uri = CloudStreamUris.canonicalize(snapshotItem.uri))
        ...
        val extras = Bundle().apply {
            ...
            putString(MediaItemBuilder.EXTERNAL_EXTRA_CONTENT_URI, item.uri)
        ...
        return MediaItem.Builder()
            ...
            .setUri(MediaItemBuilder.playbackUri(item.uri))
    ...
    private suspend fun resolveContextQueueForRequestedItem(
        ...
        val queueMediaItems = withContext(Dispatchers.IO) {
            queueSongs.map { song ->
                MediaItemBuilder.buildForExternalController(this@MusicService, song)
            }
        }.toMutableList()
    ...
    /**
     * Toggles between the currently playing item's vocals and a TAIS Studio instrumental render.
     *
     * Used to be a hard swap via [Player.replaceMediaItem] — silence the description above still
     * applies to *why* that path can't go through [MediaLibrarySession.Callback.onSetMediaItems]
     * (it always rebuilds items from [musicRepository] by mediaId, discarding any URI a client
     * attaches). But replacing the source outright meant a re-buffer/decode gap and an audible
     * cut exactly when the vocals dropped out — the opposite of what "instrumental toggle" should
     * feel like. [instrumentalCrossfadeController] keeps a second, silent player already primed
     * on the instrumental in lockstep with this one, so toggling is just a volume ramp between
     * two players already running in sync — no swap, no gap, no re-seek.
     */
    private suspend fun applyStudioAudioOverride(session: MediaSession, overridePath: String?) {
        // ExoPlayer requires main-thread access.
        withContext(Dispatchers.Main) {
            val player = session.player
            if (overridePath != null) {
                instrumentalCrossfadeController.crossfadeToInstrumental(player)
            } else {
                instrumentalCrossfadeController.crossfadeToOriginal(player)
            }
        }
    }
    ...
    private suspend fun resolveMediaItemsByIds(
        ...
        return withContext(Dispatchers.IO) {
            resolveMediaItemsWithTrustedArtworkGrants(requestedItems) { mediaId ->
