...
class LibraryScreenFolderNavigationAnimationTest {
    ...
    @Test
    fun `parent comparison handles trailing Android path separators`() {
        assertEquals(-1, resolveFolderNavigationDirection("/Music/Rock/", "/Music/"))
    }

    @Test
    fun `similar folder prefixes are not parent child relationships`() {
        assertEquals(1, resolveFolderNavigationDirection("/Music/Rock", "/Music/Roc"))
    }
