...
class LyricsUtilsTest {
    ...
    @Test
    fun parseLyrics_failedZeroWordSyncKeepsEveryOriginalLine() {
        ...
        assertEquals(listOf("First line", "Second line", "First line"), parsed.synced!!.map { it.line })
        assertEquals(6, parsed.synced!!.sumOf { it.words.orEmpty().size })
        assertTrue(parsed.synced!!.all { it.translation == null })
    }

    @Test
    fun parseLyrics_wordTimedLinesAtSameStartAreNotTranslations() {
        val raw = "[00:01.00]<00:01.00>First\n[00:01.00]<00:01.00>Second"
        val parsed = LyricsUtils.parseLyrics(raw)
        assertEquals(listOf("First", "Second"), parsed.synced!!.map { it.line })
        assertTrue(parsed.synced!!.all { it.translation == null })
    }
}
