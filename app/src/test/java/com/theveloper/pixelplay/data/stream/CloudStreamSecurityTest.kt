...
class CloudStreamSecurityTest {
    ...
    @Test
    fun `ranges reject numeric overflow and zero byte suffix`() {
        assertFalse(CloudStreamSecurity.validateRangeHeader("bytes=999999999999999999999-").isValid)
        assertFalse(CloudStreamSecurity.validateRangeHeader("bytes=-999999999999999999999").isValid)
        assertFalse(CloudStreamSecurity.validateRangeHeader("bytes=-0").isValid)
    }
