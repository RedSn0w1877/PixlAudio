...
# Lyricsfile parsing uses SnakeYAML reflectively. Keep the complete optional
# parser package in release builds; obfuscating its package metadata can make
# Resolver initialization fail on Android and crash lyric loading.
...
# Gson persists this private repository DTO using its unannotated field names. It is
# outside data.model/data.network; preserve existing cache keys across release updates.
-keep class com.theveloper.pixelplay.data.repository.LyricsData {
    <fields>;
    <init>(...);
}

# The home catalog now caches this nested Gson value; the container's @Keep does
# not propagate to its field types. Keep its unannotated persisted keys as well.
-keep class com.theveloper.pixelplay.data.youtube.YouTubeSearchResult {
    <fields>;
    <init>(...);
}
...
# LiteRT 2.2 liblitert_jni.so constructs these types and resolves ElementType enum
# fields by their literal names. The AAR's consumer rules cover only legacy
# org.tensorflow.lite.annotations.UsedByReflection, not these new API classes.
# AGP's default native-method rule already protects CompiledModel/Environment/
# TensorBuffer JNI entry points; retain only the additional native-created types.
...
-keep class com.google.ai.edge.litert.TensorType$ElementType { *; }
-keep class com.google.ai.edge.litert.TensorType$Layout { *; }
